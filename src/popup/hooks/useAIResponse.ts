...
import { AIResponse, ResponseMetadata, GenerateAIRequest } from '../../shared/types/ai-response';
...
export const useAIResponse = () => {
  ...
  const generateResponse = useCallback(async (
    ...
    try {
      ...
      const result: AIResponse = await apiClient.post('/api/response/generate', request);
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to generate AI response';
  ...
  const retryGeneration = useCallback(async (): Promise<void> => {
    ...
    while (retries <= maxRetries) {
      try {
        ...
        const result: AIResponse = await apiClient.post('/api/response/generate', lastRequestRef.current);
